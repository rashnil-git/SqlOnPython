...
class Database(dict):
    ...
    def __getattribute__(self, item):
        try:
            ...
            if item not in ['transactionId', 'transactions'] and 'transactionId' in dict.__getattribute__(self, '__dict__') and item in dict.__getattribute__(self, 'transactions')[dict.__getattribute__(self, 'transactionId')]:
    ...
    def _preDump(self, d):
        ...
        for k in d:
            if isinstance(d[k], Relation):
                if isinstance(d[k]._body, collections.Callable):    #todo? and, therefore?, or just if, in self.__dict__
                    del res[k]
        return res
    ...
    def __contains__(self, item):
        if item not in ['transactionId', 'transactions'] and 'transactionId' in self.__dict__ and item in self.transactions[self.transactionId] and isinstance(self.transactions[self.transactionId][item], Relation):
           return True
    ...
    def _dump(self):
        ...
        if dumpDebug:
            ...
            for (r,v) in self:
                ...
                f.write("%s = %s\n\n" % (r,repr(v)))        #todo prefix relation name with self.
        ...
        for r in self.__dict__:
            if r=='_readonly':
                ...
                if r=="transactionId":
                    ...
                    if isinstance(odict[r]._body, collections.Callable):
...
#            return [y for x in k for y in tl if y['order']==x]
    ...
    def vconstraints(self):
        return [Tuple(relation_name=k, constraint_name="%s_%s" % (k, rn), constraint_type=rf.__name__)
                for (k, v) in self
